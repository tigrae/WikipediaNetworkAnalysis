...
from util.classes import load_all_categories, Article


def draw_horizontal_bar_chart(plot_dict, title="", xlabel="", ylabel=""):
    fig, ax = plt.subplots(figsize=(10, 6))
    bars = ax.barh(list(plot_dict.keys())[::-1], list(plot_dict.values())[::-1], color='blue')
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    plt.tight_layout()
...
def draw_category_article_number(categories, n=0):
    ...
    for category in categories:
        cat_article_count[category.title] = len(category.articles)
    ...
    if n > 0:
        ...
        for category in categories:
            related_article_count[category.title] = 0
            for article_title in category.articles:
                ...
                related_article_count[category.title] += len(_current_article.relations)
...
if __name__ == "__main__":
    ...
    draw_category_article_number(categories, n=1)
